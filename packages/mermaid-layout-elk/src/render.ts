...
interface NodeWithVertex extends Omit<Node, 'domId'> {
  ...
  // The plain-string domId `getData()` originally assigned this node, captured before
  // `domId` above gets repurposed to hold a live D3 selection instead (see addVertex) — a
  // group node's own rendering (insertCluster -> clusters.js) needs the real string to set
  // the cluster's DOM `id` attribute; setting it to a Selection there just stringifies to
  ...
  const addVertex = async (
    ...
    if (!node.isGroup) {
      ...
      const child: NodeWithVertex & { children: NodeWithVertex[] } = {
        ...node,
        domId: undefined,
        // node.domId is still the plain string getData() assigned at this point — grab it
        // before it's cleared above (and before this same field gets reused below to hold
        // a live D3 selection instead).
        stringDomId: node.domId,
        children: [],
      };
      ...
      nodeArray.map(async function (node: {
        ...
        if (node) {
          ...
          if (node.isGroup) {
            ...
            // node.domId is a live D3 selection by this point (see addVertex above — a group
            // node's domId gets repurposed to the container element itself, since edges that
            // touch a subgraph need `.domId.node().getBBox()` later in this file). clusters.js
            // just does `.attr('id', node.domId)` to set the cluster's real DOM id, expecting a
            // plain string — handed the selection, d3 stringifies it as the literal text
            // "[object Object]", breaking any code (like the playground editor's
            // click-to-select) that reads a cluster's id back. Use the plain string captured
            // in addVertex instead, before domId was repurposed.
            clusterNode.domId = node.stringDomId;
