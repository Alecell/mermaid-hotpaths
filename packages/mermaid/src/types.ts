export interface NodeMetaData {
  ...
  ticket?: string;
  labelType?: string;
  /** Hotpaths: this node declares a single trigger (its color is given by `color`). */
  trigger?: string;
  /**
   * Hotpaths: several triggers on one node. In flowchart syntax use brace-free
   * strings `["name:#color", ...]`; the object form is for the JS API only.
   */
  triggers?: (string | { name: string; color?: string })[];
  /** Hotpaths: color for the trigger declared via `trigger` / bare `triggers` strings. */
  color?: string;
  /** Hotpaths: trigger name(s) this node listens to (lights up passively). */
