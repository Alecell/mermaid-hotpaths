...
import {
  applyHotpaths,
  parseHotpathMetadata,
  type HotpathData,
  type HotpathNodeInfo,
} from './hotpaths.js';
import type {
  FlowClass,
  FlowEdge,
...
export class FlowDB implements DiagramDB {
  ...
  constructor() {
    ...
    this.funs.push(this.setupHotpaths.bind(this));
  ...
  public addVertex(
    ...
    // Hotpaths: capture trigger/listen metadata (no-op when absent).
    const hotpath = parseHotpathMetadata(doc);
    if (hotpath) {
      if (vertex.hotpath) {
        vertex.hotpath.declares.push(...hotpath.declares);
        vertex.hotpath.listens.push(...hotpath.listens);
      } else {
        vertex.hotpath = hotpath;
      }
    }
  }
...
You have to call mermaid.initialize.`
  ...
  /**
   * Hotpaths: aggregate the per-node trigger metadata into a renderer-friendly shape.
   * Builds the registry of declared triggers (name -\> color) and, per vertex, which
   * triggers it sources (declares) and which it belongs to (declares ∪ listens).
   */
  public getHotpaths(): HotpathData {
    const triggers = new Map<string, string | undefined>();
    const nodes = new Map<string, HotpathNodeInfo>();
    for (const vertex of this.vertices.values()) {
      const hp = vertex.hotpath;
      if (!hp) {
        continue;
      }
      const declares: string[] = [];
      for (const t of hp.declares) {
        if (!declares.includes(t.name)) {
          declares.push(t.name);
        }
        if (t.color) {
          const prev = triggers.get(t.name);
          if (prev && prev !== t.color) {
            log.warn(
              `Hotpaths: trigger "${t.name}" was redeclared with a different color ("${prev}" -> "${t.color}"). Using the latest.`
            );
          }
          triggers.set(t.name, t.color);
        } else if (!triggers.has(t.name)) {
          triggers.set(t.name, undefined);
        }
      }
      const member = [...new Set<string>([...declares, ...hp.listens])];
      nodes.set(vertex.id, { domId: vertex.domId, declares, member });
    }
    return { triggers, nodes };
  }

  private setupHotpaths(element: Element) {
    applyHotpaths(element, this);
  }
  ...
  public clear(ver = 'gen-2') {
    ...
    this.edges = [];
    this.funs = [this.setupToolTips.bind(this), this.setupHotpaths.bind(this)];
    this.diagramId = '';
    this.subGraphs = [];
